...
exports.LLMBasedTagger = void 0;
const ai_1 = require("ai");
const schemas_1 = require("../../src/lib/schemas");
const ConfigManager_1 = require("../../src/lib/ConfigManager");
const OpenAIClient_1 = require("../../src/lib/OpenAIClient");
class LLMBasedTagger {
    ...
    async tagRecipe(rawRecipe) {
        try {
            ...
            const openaiProvider = (0, OpenAIClient_1.createOpenAIProvider)(modelConfig);
            const model = (0, OpenAIClient_1.getModelFromProvider)(openaiProvider, modelConfig.model);
            const result = await (0, ai_1.generateObject)({
                model: model,
                system: this.getSystemPrompt(),
                prompt: this.buildPrompt(rawRecipe),
                schema: schemas_1.RecipeTagsSchema,
    ...
    async tagRecipesIncremental(rawRecipes, existingData = {}) {
...
注意：每个字段都应该是数组，即使只有一个值。如果某个特征不明确或不适用，可以返回空数组。`;
    ...
    estimateDifficulty(recipe) {
        ...
        if (ingredientLines.length > 15)
            difficulty += 1;
        else if (ingredientLines.length > 10)
            difficulty += 0.5;
    ...
    generateFallbackTags(recipe) {
        ...
        // 推断烹饪方式
        if (allText.includes('炒'))
            tags.cookingStyle.push('炒');
        if (allText.includes('蒸'))
            tags.cookingStyle.push('蒸');
        if (allText.includes('炖') || allText.includes('煨'))
            tags.cookingStyle.push('炖');
        if (allText.includes('炸'))
            tags.cookingStyle.push('炸');
        if (allText.includes('凉拌') || allText.includes('拌'))
            tags.cookingStyle.push('凉拌');
        if (allText.includes('烤'))
            tags.cookingStyle.push('烤');
        if (allText.includes('烧') || allText.includes('红烧'))
            tags.cookingStyle.push('烧');
        if (allText.includes('焖'))
            tags.cookingStyle.push('焖');
        if (allText.includes('煮') || allText.includes('水煮'))
            tags.cookingStyle.push('煮');
        if (allText.includes('煎'))
            tags.cookingStyle.push('煎');
        if (allText.includes('烙'))
            tags.cookingStyle.push('烙');
        if (allText.includes('汆'))
            tags.cookingStyle.push('汆');
        ...
        const hasDangerousIngredients = dangerousIngredients.some(ingredient => allText.includes(ingredient));
...
exports.LLMBasedTagger = LLMBasedTagger;
//# sourceMappingURL=LLMBasedTagger.js.map